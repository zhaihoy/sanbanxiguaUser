...
//                    }
    ...
    private void addSingleItem(LabelBean labelBean) {
        ...
        item.setOptionIcon(R.drawable.sharer_red_circle)
                ...
                .setRadioButtonDimension(18)
                .setRadioButtonMarginLeft(10)
                .setTextSize(14);
...
//        String[] params = processRequestParams();

//        if (httpUtil == null)
//            httpUtil = new HttpUtil();
//        httpUtil.callJson(handler, RETRIEVE_SUBMIT_NEW_ITEM, this, GlobalConfig.SHARER_SERVER_ROOT + GlobalConfig.SUBMIT_NEW_ITEM, BaseResultBean.class,params);
...
//        String picList = assemblePicsInBase64();
        ...
        return new String[]{Constants.ECID, GlobalConfig.ECID, Constants.PhoneNumber, phoneNumber,
                Constants.GoodsSNID,goodsSNID==null?"":goodsSNID,
                Constants.PicList, picList, Constants.INTRODUCTION, et_intro.getText().toString().trim(),
    ...
    /**
     * 按Base64编码所有的图片,返回json串 "["a","b"]"
     */
    private String assemblePicsInBase64() {

//        new BitmapEncodeTask().execute(adapter.data);
        ...
        for (int i = 0; i < adapter.data.size(); i++) {
            ...
            Bitmap bitmap = BitmapFactory.decodeFile(adapter.data.get(i));
            bitmap.compress(Bitmap.CompressFormat.JPEG, 100, baos);
            String encodeToString = Base64.encodeToString(baos.toByteArray(), Base64.DEFAULT);

            Log.d(TAG, "assemblePicsInBase64: jsonArray===>>>" + encodeToString.length());
    ...
    private String assembleLabelContent() {
        ...
        for (int i = 0; i < childCount - 1; i++) {
            ...
            sb.append(labelBean.getGoodsTypeLabelID()).append(",")
...
//                        scrollView.scrollTo(0, 200);
                        ...
                        if(labelBeans!=null&&labelBeans.size()>0){
                            ...
                            for (int i = 0; i <labelBeans.size(); i++) {
                                ...
                                if(label.getGoodsTypeID()!=null
                                        &&label.getGoodsTypeID().equals(typeID)){
    ...
    private void showRationale() {
        ...
        new AlertDialog.Builder(this).setCancelable(true).setTitle("您未授予此应用SD卡访问权限")
                .setMessage("请到系统设置页面授予该应用此权限后,再调用此功能").show();
