...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run() {
			...
			/**
			 * Only set startTime if this is the first fxzc_time we're starting, else
	...
	private void init(Context context, AttributeSet attrs) {
		...
		String pullLabel = "下拉可以刷新";
		String refreshingLabel = "加载中...";
		String releaseLabel ="松开可以刷新";

		// Add Loading Views
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			headerLayout = new PullLoadingLayout(context,
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			footerLayout = new PullLoadingLayout(context, MODE_PULL_UP_TO_REFRESH,
					"松手可以加载", "上拉可以加载",
					refreshingLabel);
	...
	public interface OnRefreshListener {
		...
		void onRefresh(int refresh_mode);
