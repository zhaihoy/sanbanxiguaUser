...
import com.hbbc.mshare.user.zhijin_section.FailOutPaperActivity;
import com.hbbc.mshare.user.zhijin_section.OutPaperResultProcess;
...
//        super.onDestroy();
    ...
    private void handleRealPayResult(Message msg) {
        ...
        if("DEPOSIT".equals(type)){//押金的支付结果
            ...
            if(payStatus==1){
                result="订单支付成功";
                //支付成功之后要请求去访问—>读条—>开锁接口
                openActivity(OutPaperResultProcess.class);
            }else{
                openActivity(FailOutPaperActivity.class);
            }
