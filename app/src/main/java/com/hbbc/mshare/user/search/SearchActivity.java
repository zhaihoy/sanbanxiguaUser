...
public class SearchActivity extends BaseActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_cancel.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                btn_cancel.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        finish();
                    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        if (TextUtils.isEmpty(input)) {
            ...
            if(tipList.get(position).getPoint()!=null){
                ...
                ToastUtil.toast("该地址无效,请重新选择");
                return;
            }

            Log.d("tag", "onItemClick: tip-->"+tipList.get(position).getPoint());
            Log.d("tag", "onItemClick: "+tipList.get(position).getPoiID());
            Log.d("tag", "onItemClick: "+tipList.get(position).getTypeCode());
    ...
    private void saveSelectedTip2DB(Object item) {
        ...
        if (id != -1) {
            ...
            ToastUtil.toast_debug("update_columns===" + update);
        } else
            dao.insert(bean);
    }
