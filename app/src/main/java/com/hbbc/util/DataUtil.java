...
public class DataUtil {
    ...
    /**
     * 得到当前日期的字符串表示。
     * eg: System.out.println(DataUtil.getNowDateString());
     *
    ...
    /**
     * 与当前时间的时间差
     * eg: DataUtil.getOldTime2New("2016-12-12 10:10:10");
     *
     * @param oldtime 2016-12-12 10:10:10
     */
    public static String getOldTime2New(String oldtime) {

        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String oldTime = format.format(oldtime);
        String newtime = format.format(new Date());

        String value = "";
        long oldT = Long.parseLong(oldtime);
        long newT = Long.parseLong(newtime);


        long D_value = newT - oldT;
        if (D_value > 0) {
            //年
            int d_year = (int) (D_value / (12 * 30 * 24 * 60 * 60 * 1000));
            //月
            int d_month = (int) (D_value / (30 * 24 * 60 * 60 * 1000));
            //天
            int d_day = (int) (D_value / (24 * 60 * 60 * 1000));
            //小时
            int d_hour = (int) (D_value / (60 * 60 * 1000));
            //分钟
            int d_minute = (int) (D_value / (60 * 1000));
            //秒
            int d_second = (int) (D_value / (1000));
            if (d_year > 0) {
                value = d_year + "年之前";
            } else if (d_month > 0) {
                value = d_month + "月之前";
